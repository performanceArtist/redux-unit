import { HandlerMap, FlatHandlerMap, AnyHandler } from '../types';
import { isApiHandler, getApiActionTypes } from '../api';
import { TypeFormatter } from '../type-formatter';

export function makeHandlerMap<S extends object, M extends HandlerMap<S>>(
  model: M,
  ...
  return Object.keys(model).reduce<FlatHandlerMap<S>>((acc, key) => {
    ...
    if (isApiHandler(handler)) {
      ...
      if (handler.request && actions.request)
        acc[actions.request] = handler.request;
      if (handler.reset && actions.reset) acc[actions.reset] = handler.reset;
      if (handler.failure && actions.failure)
        acc[actions.failure] = handler.failure;
    } else {
      acc[typeFormatter(key)] = handler as AnyHandler<S>;
    }
